var imgRef = [];
var getImg = [];
let str = localStorage.getItem("queryImg");
if (str) {
  getImg = JSON.parse(str);
}

var text_lst;

function deleteImage(elem) {
  $(elem).parent('.data').remove();
  updateLocalStorage();
  if ($("#localImg")[0].firstElementChild == null) {
    $(".margin-searchHeader #localImg").text("Nothing selected yet") || ""
  }
}
...
function updateLocalStorage() {
  ...
  data.each(function () {
    let src = $(this)[0].firstElementChild.currentSrc;
    let name = $(this)[0].children[1].innerText;
    getImg.push({ url: src, name: name });
  });
  var chacheItemString = JSON.stringify(getImg);
  localStorage.setItem("queryImg", chacheItemString)
}

function addToSearchO(e) {
...
function remove() {
  ...
  imgRef = [];
  localStorage.setItem("queryRef", imgRef);
  $("#localRef").text("Nothing selected yet");
}

$(document).ready(function () {

// ! temporarily switched off for fake text in results.html
 // $("#localFilter").text(localStorage.getItem("queryFilter"));

  // $("#localInput").text(localStorage.getItem("queryInput"));

  var queryRef = localStorage.getItem("queryRef");

  if (queryRef) {
    $("#localRef").text(queryRef);
    $('<span class="click-me">x</span>').appendTo('#localRef').click(remove);
  }
  ...
  if (getImg) {
    if (getImg.length == 0) {
      ...
      for (i = 0; i < getImg.length; i++) {
        $('#localImg').append(`<div class="data">
          <img src="${getImg[i].url}"> 
          <p>${getImg[i].name}</p>
          <p id="${getImg[i].name}" class="delete-image" onclick="deleteImage(this), updateLocalStorage()">x</p>
        </div>`);
      }
    }
  }
  ...
  var PhotoViewer = /*#__PURE__*/ function () {
    ...
    _createClass(PhotoViewer, [{
      ...
      value: function _createBtns(toolbar) {
        ...
        var btns = ['minimize', 'maximize', 'caption', 'addToSearch', 'close', 'zoomIn', 'zoomOut', 'prev', 'next', 'fullscreen', 'actualSize', 'rotateLeft', 'rotateRight'];
        var btnsHTML = '';
    ...
    },
    {
      ...
      value: function build() {
        ...
        this.$legend = $photoviewer.find(CLASS_NS + '-legend');
        this.$image = $photoviewer.find(CLASS_NS + '-image');
        this.$addToSearch = $photoviewer.find(CLASS_NS + '-button-addToSearch');
        this.$caption = $photoviewer.find(CLASS_NS + '-button-caption');
      ...
      value: function setModalSize(img) {
        ...
        var modalWidth = 400 + this._stageEdgeValue.horizontal;
        var modalHeight = 400 + this._stageEdgeValue.vertical; // Extra width/height for `content-box`
    ...
    },
    {
      ...
      value: function _addEvents() {
        ...
        this.$caption.off(CLICK_EVENT + EVENT_NS).on(CLICK_EVENT + EVENT_NS, function () {
          _this5.caption();
        });
